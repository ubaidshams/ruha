...
// Get viral products - Must be before /:id route
router.get("/viral", async (req, res) => {
  try {
    ...
    console.error("Get viral products error:", error);
    res.status(500).json({
      success: false,
      message: "Failed to fetch viral products",
      error: error.message,
    });
  }
});
...
// Get blind box contents (admin only) - Must be before /:id route
router.get(
  "/:id/contents",
  [
    auth,
...
// Add a review to a product - Must be before /:id route
router.post(
  "/:id/reviews",
  [
    auth,
